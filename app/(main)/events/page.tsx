...
// Revalidate the public listing periodically (events change infrequently)
...
async function getEvents(): Promise<ApiEvent[]> {
  try {
    ...
    console.error("Failed to load events for listing:", err);
    return [];
  }
}
              ...
              return (
                <div key={section.key} className="mb-12">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {items.map((event) => (
                      <Card key={event.id ?? event.slug} className="p-5 sm:p-6 border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col">
